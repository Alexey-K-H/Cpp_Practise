...
protected:
    ...
    int shipRow;
    char shipColumn;
    std::vector<int> ship_size;//Содержит размеры кораблей
public:
    Gamer();
    ...
    virtual void Fire(int row, char column) = 0;//Принять огонь противника на свое поле
...
public:
    ...
    void Fire(int row, char column) override;
};
