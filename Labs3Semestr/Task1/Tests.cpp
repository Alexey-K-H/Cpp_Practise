...
TEST_CASE("Create set")
{
        ...
        REQUIRE_THROWS_AS(Trait_set(-5), std::invalid_argument);
        REQUIRE_THROWS_AS(Trait_set(-15), std::invalid_argument);
}

//Проверка работы опреаторов и методов основного класса
TEST_CASE("Operators for Trait_set class")
{
    SECTION("[]")
    {
        Trait_set test(5);
    ...
    SECTION("COPY")
    {
        Trait_set test_for_copy(5);
        Trait_set copy = test_for_copy;
    ...
    SECTION("&")
    {
        Trait_set set_AND_1(3);
        Trait_set set_AND_2(7);

        set_AND_1 = True;
        set_AND_2 = False;

        Trait_set result_of_AND = set_AND_1 & set_AND_2;
    ...
    SECTION("~")
    {
        Trait_set set_primal(3);
        set_primal[0] = True;
        set_primal[1] = Unknown;
        set_primal[2] = False;

        Trait_set set_not = ~set_primal;
    ...
    {
        Trait_set test_stat_gen(5);
        test_stat_gen[0] = True;
        test_stat_gen[1] = False;
        test_stat_gen[2] = False;
        test_stat_gen[6] = True;
        std::unordered_map<Trait , int> stat = test_stat_gen.Cardinality();
        REQUIRE(stat[True] == 2);
